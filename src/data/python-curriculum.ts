...
\`\`\``,
    ...
    {
      id: 'milestone-1-basics',
      title: 'Milestone: Basics Challenge',
      description: 'Put your fundamentals to the test',
      icon: '\uD83C\uDFC6',
      lessons: [
        {
          id: 'challenge-password-validator',
          title: 'Build a Password Validator',
          type: 'challenge',
          xp: 75,
          challenge: {
            description: 'Create a function that validates passwords based on common security rules.\n\nA valid password must:\n- Be at least 8 characters long\n- Contain at least one uppercase letter\n- Contain at least one lowercase letter\n- Contain at least one digit\n\nReturn True if the password is valid, False otherwise.',
            difficulty: 'easy',
            examples: [
              { input: '"MyPass1word"', output: 'True', explanation: 'Has 11 chars, uppercase, lowercase, and a digit' },
              { input: '"short1A"', output: 'False', explanation: 'Only 7 characters (needs at least 8)' },
              { input: '"alllowercase1"', output: 'False', explanation: 'Missing an uppercase letter' },
              { input: '"ALLUPPERCASE1"', output: 'False', explanation: 'Missing a lowercase letter' },
            ],
            functionName: 'validate_password',
            starterCode: 'def validate_password(password):\n    # Check all the rules and return True or False\n    pass\n',
            solution: 'def validate_password(password):\n    if len(password) < 8:\n        return False\n    has_upper = any(c.isupper() for c in password)\n    has_lower = any(c.islower() for c in password)\n    has_digit = any(c.isdigit() for c in password)\n    return has_upper and has_lower and has_digit',
            testCases: [
              { input: '"MyPass1word"', expected: 'True', description: 'Valid password with all requirements' },
              { input: '"short1A"', expected: 'False', description: 'Too short (7 chars)' },
              { input: '"alllowercase1"', expected: 'False', description: 'Missing uppercase letter' },
              { input: '"ALLUPPERCASE1"', expected: 'False', description: 'Missing lowercase letter' },
              { input: '"NoDigitsHere"', expected: 'False', description: 'Missing digit' },
              { input: '"Abcdefg1"', expected: 'True', description: 'Exactly 8 chars, all rules met' },
              { input: '""', expected: 'False', description: 'Empty string' },
            ],
            hints: [
              'Use len() to check the password length',
              'String methods like .isupper(), .islower(), .isdigit() check individual characters',
              'The any() function with a generator expression can check if at least one character meets a condition: any(c.isupper() for c in password)',
            ],
          },
        },
        {
          id: 'challenge-temperature-converter',
          title: 'Temperature Converter',
          type: 'challenge',
          xp: 50,
          challenge: {
            description: 'Create a function that converts temperatures between Celsius, Fahrenheit, and Kelvin.\n\nThe function takes a temperature value, the scale it is in, and the target scale.\n\nFormulas:\n- C to F: (C * 9/5) + 32\n- F to C: (F - 32) * 5/9\n- C to K: C + 273.15\n- K to C: K - 273.15\n\nReturn the result rounded to 2 decimal places.',
            difficulty: 'easy',
            examples: [
              { input: '100, "C", "F"', output: '212.0', explanation: 'Boiling point of water' },
              { input: '32, "F", "C"', output: '0.0', explanation: 'Freezing point of water' },
              { input: '0, "C", "K"', output: '273.15', explanation: 'Celsius to Kelvin' },
            ],
            functionName: 'convert_temp',
            starterCode: 'def convert_temp(value, from_scale, to_scale):\n    # Convert between "C", "F", and "K"\n    pass\n',
            solution: 'def convert_temp(value, from_scale, to_scale):\n    # First convert to Celsius\n    if from_scale == "C":\n        celsius = value\n    elif from_scale == "F":\n        celsius = (value - 32) * 5 / 9\n    else:\n        celsius = value - 273.15\n    # Then convert from Celsius to target\n    if to_scale == "C":\n        return round(celsius, 2)\n    elif to_scale == "F":\n        return round(celsius * 9 / 5 + 32, 2)\n    else:\n        return round(celsius + 273.15, 2)',
            testCases: [
              { input: '100, "C", "F"', expected: '212.0', description: '100C = 212F (boiling point)' },
              { input: '32, "F", "C"', expected: '0.0', description: '32F = 0C (freezing point)' },
              { input: '0, "C", "K"', expected: '273.15', description: '0C = 273.15K' },
              { input: '0, "K", "C"', expected: '-273.15', description: '0K = -273.15C (absolute zero)' },
              { input: '98.6, "F", "C"', expected: '37.0', description: '98.6F = 37C (body temperature)' },
              { input: '50, "C", "C"', expected: '50.0', description: 'Same scale returns same value' },
            ],
            hints: [
              'Convert to Celsius first as an intermediate step, then convert from Celsius to the target scale',
              'Use if/elif to handle each from_scale, then another if/elif for the to_scale',
              'Remember to use round(value, 2) to round to 2 decimal places',
            ],
          },
        },
      ],
...
\`\`\``,
    ...
    {
      id: 'milestone-2-data',
      title: 'Milestone: Data Processing',
      description: 'Combine your data skills',
      icon: '\uD83C\uDFC6',
      lessons: [
        {
          id: 'challenge-shopping-cart',
          title: 'Shopping Cart Calculator',
          type: 'challenge',
          xp: 75,
          challenge: {
            description: 'Build a function that calculates the total cost of a shopping cart.\n\nEach item is a dictionary with "name", "price", and "quantity" keys.\nThe function also takes a discount percentage (0-100).\n\nReturn the total after applying the discount, rounded to 2 decimal places.',
            difficulty: 'medium',
            examples: [
              { input: '[{"name": "Apple", "price": 1.5, "quantity": 3}, {"name": "Bread", "price": 2.99, "quantity": 1}], 10', output: '6.3', explanation: 'Subtotal = 4.5 + 2.99 = 7.49, 10% off = 6.741, rounded = 6.74... actually (7.49 * 0.9) = 6.741' },
              { input: '[], 0', output: '0.0', explanation: 'Empty cart = 0' },
            ],
            functionName: 'calculate_cart',
            starterCode: 'def calculate_cart(items, discount_percent):\n    # Calculate subtotal, apply discount, return total\n    pass\n',
            solution: 'def calculate_cart(items, discount_percent):\n    subtotal = sum(item["price"] * item["quantity"] for item in items)\n    total = subtotal * (1 - discount_percent / 100)\n    return round(total, 2)',
            testCases: [
              { input: '[{"name": "Apple", "price": 1.5, "quantity": 3}, {"name": "Bread", "price": 2.99, "quantity": 1}], 10', expected: '6.74', description: 'Two items with 10% discount' },
              { input: '[], 0', expected: '0.0', description: 'Empty cart' },
              { input: '[{"name": "Laptop", "price": 999.99, "quantity": 1}], 0', expected: '999.99', description: 'Single item, no discount' },
              { input: '[{"name": "Pen", "price": 0.99, "quantity": 10}], 50', expected: '4.95', description: '50% off 10 pens' },
              { input: '[{"name": "A", "price": 10, "quantity": 2}, {"name": "B", "price": 5, "quantity": 4}], 25', expected: '30.0', description: 'Multiple items with 25% discount' },
            ],
            hints: [
              'Use sum() with a generator to calculate the subtotal: sum(item["price"] * item["quantity"] for item in items)',
              'Apply discount by multiplying: subtotal * (1 - discount_percent / 100)',
              'Use round(total, 2) for the final result',
            ],
          },
        },
        {
          id: 'challenge-word-frequency',
          title: 'Word Frequency Counter',
          type: 'challenge',
          xp: 75,
          challenge: {
            description: 'Create a function that counts word frequencies in a text.\n\nThe function should:\n- Convert all words to lowercase\n- Remove punctuation (.,!?;:)\n- Return a dictionary mapping each word to its count\n- Words are separated by spaces',
            difficulty: 'medium',
            examples: [
            ...
            testCases: [
              { input: '"Hello world hello"', expected: '{"hello": 2, "world": 1}', description: 'Basic word counting (case insensitive)' },
              { input: '"The cat, the dog!"', expected: '{"the": 2, "cat": 1, "dog": 1}', description: 'Handles punctuation' },
              { input: '""', expected: '{}', description: 'Empty string returns empty dict' },
              { input: '"one"', expected: '{"one": 1}', description: 'Single word' },
              { input: '"a a a b b c"', expected: '{"a": 3, "b": 2, "c": 1}', description: 'Multiple frequencies' },
            ],
            hints: [
              'Use str.replace() to remove each punctuation character one at a time',
              'Use str.lower() before splitting to make it case-insensitive',
              'dict.get(key, 0) returns 0 if the key does not exist — useful for counting',
            ],
          },
        },
      ],
...
\`\`\``,
    ...
    {
      id: 'milestone-3-intermediate',
      title: 'Milestone: Real-World Challenge',
      description: 'Solve practical problems',
      icon: '\uD83C\uDFC6',
      lessons: [
        {
          id: 'challenge-csv-parser',
          title: 'CSV Data Analyzer',
          type: 'challenge',
          xp: 100,
          challenge: {
            description: 'Create a function that parses CSV-formatted text and returns summary statistics.\n\nThe CSV has a header row and data rows. Given a column name, return a dictionary with:\n- "count": number of non-empty values\n- "values": list of all values in that column (as strings)\n\nRows are separated by newlines, columns by commas.',
            difficulty: 'medium',
            examples: [
              { input: '"name,age,city\\nAlice,30,NYC\\nBob,25,LA\\nCharlie,35,NYC", "age"', output: '{"count": 3, "values": ["30", "25", "35"]}' },
              { input: '"name,score\\nAlice,90\\nBob,", "score"', output: '{"count": 1, "values": ["90"]}', explanation: 'Bob has empty score, not counted' },
            ],
            functionName: 'analyze_csv',
            starterCode: 'def analyze_csv(csv_text, column_name):\n    # Parse the CSV and analyze the given column\n    pass\n',
            solution: 'def analyze_csv(csv_text, column_name):\n    lines = csv_text.strip().split("\\n")\n    headers = lines[0].split(",")\n    col_idx = headers.index(column_name)\n    values = []\n    for line in lines[1:]:\n        cols = line.split(",")\n        if col_idx < len(cols) and cols[col_idx].strip():\n            values.append(cols[col_idx].strip())\n    return {"count": len(values), "values": values}',
            testCases: [
              { input: '"name,age,city\\nAlice,30,NYC\\nBob,25,LA", "age"', expected: '{"count": 2, "values": ["30", "25"]}', description: 'Extract age column' },
              { input: '"name,age,city\\nAlice,30,NYC\\nBob,25,LA", "city"', expected: '{"count": 2, "values": ["NYC", "LA"]}', description: 'Extract city column' },
              { input: '"name,score\\nAlice,90\\nBob,", "score"', expected: '{"count": 1, "values": ["90"]}', description: 'Handles empty values' },
              { input: '"x\\n1\\n2\\n3", "x"', expected: '{"count": 3, "values": ["1", "2", "3"]}', description: 'Single column CSV' },
            ],
            hints: [
              'Split the text by newlines to get rows, then split each row by commas to get columns',
              'The first row contains headers — use .index() to find the column position',
              'Skip empty values when building the values list: check if the string is non-empty after .strip()',
            ],
          },
        },
        {
          id: 'challenge-grade-calculator',
          title: 'Student Grade Calculator',
          type: 'challenge',
          xp: 100,
          challenge: {
            description: 'Create a function that takes a list of student dictionaries and returns the class statistics.\n\nEach student has "name" and "grades" (list of numbers).\n\nReturn a dictionary with:\n- "class_average": average of all student averages (rounded to 1 decimal)\n- "top_student": name of student with highest average\n- "passing": number of students with average >= 60',
            difficulty: 'hard',
            examples: [
              { input: '[{"name": "Alice", "grades": [90, 80, 70]}, {"name": "Bob", "grades": [50, 60, 40]}]', output: '{"class_average": 65.0, "top_student": "Alice", "passing": 1}', explanation: 'Alice avg=80, Bob avg=50. Class avg=65. Only Alice passes.' },
            ],
            functionName: 'class_stats',
            starterCode: 'def class_stats(students):\n    # Calculate class statistics\n    pass\n',
            solution: 'def class_stats(students):\n    averages = []\n    for s in students:\n        avg = sum(s["grades"]) / len(s["grades"])\n        averages.append((s["name"], avg))\n    class_avg = sum(a[1] for a in averages) / len(averages)\n    top = max(averages, key=lambda x: x[1])\n    passing = sum(1 for _, avg in averages if avg >= 60)\n    return {"class_average": round(class_avg, 1), "top_student": top[0], "passing": passing}',
            testCases: [
              { input: '[{"name": "Alice", "grades": [90, 80, 70]}, {"name": "Bob", "grades": [50, 60, 40]}]', expected: '{"class_average": 65.0, "top_student": "Alice", "passing": 1}', description: 'Two students, one passing' },
              { input: '[{"name": "Zoe", "grades": [100, 100]}]', expected: '{"class_average": 100.0, "top_student": "Zoe", "passing": 1}', description: 'Single perfect student' },
              { input: '[{"name": "A", "grades": [60]}, {"name": "B", "grades": [60]}, {"name": "C", "grades": [59]}]', expected: '{"class_average": 59.7, "top_student": "A", "passing": 2}', description: 'Edge case: exactly 60 passes' },
              { input: '[{"name": "X", "grades": [70, 80, 90]}, {"name": "Y", "grades": [80, 85, 75]}, {"name": "Z", "grades": [95, 92, 88]}]', expected: '{"class_average": 83.9, "top_student": "Z", "passing": 3}', description: 'Three students all passing' },
            ],
            hints: [
              'Calculate each student\'s average first: sum(grades) / len(grades)',
              'Use max() with a key function to find the top student',
              'Count passing students using sum() with a condition: sum(1 for ... if avg >= 60)',
            ],
          },
        },
      ],
